    ...
    for(int k=0;k<Ny;k++)
    {
        for(int i=0;i<Nx;i++)
        {
            ...
            for(int j=0;j<QPT_origin.size();j++)
                if(ind_h!=ind)
                    SR[ind_h][j]=QPT_origin[j]*s*SR[ind_h].diff_norm*SR[ind_h].diff_norm+SHIFT;
                else
...
void layer_koh::learnW(const QVector<float>& inp)
{
    ...
    for(int i=0;i<N;i++)
    {
        ...
        float h_func=exp(-h1/(1500000*exp_val*exp_val+0.00001));//.0000001
